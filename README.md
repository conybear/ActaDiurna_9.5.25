...
| Layer      | Technology                                         |
|------------|----------------------------------------------------|
| Frontend   | React 19, Tailwind CSS 3, Shadcn/UI (Radix UI)    |
| Backend    | Python, FastAPI 0.110, Uvicorn                     |
| Database   | MongoDB (via Motor 3.3 async driver)               |
| Auth       | JWT (PyJWT 2.10), bcrypt 4.1 (passlib)             |
| Email      | Resend 2.19 (verified domain: conybear.com)        |
| Storage    | Cloudinary 1.44                                    |
| Forms      | React Hook Form 7, Zod 3                           |
| Routing    | React Router DOM 7                                 |
| Build      | CRACO, react-scripts 5                             |
...
- Add environment variable: `REACT_APP_BACKEND_URL=https://api.conybear.com`
- Add custom domain: `conybear.com`
...
- Add custom domain: `api.conybear.com`

### DNS (at your registrar)
- Apex domain (`conybear.com`) → follow Vercel's DNS instructions
- `CNAME api` → your Render service URL
...
- `APP_URL` must point to the deployed frontend URL so email invitation links work correctly.
- Emails are sent from `noreply@conybear.com` via Resend using a verified custom domain.
- Preview and production environments should use separate MongoDB databases.
- The `bcrypt` version is pinned to `4.1.3` in `requirements.txt` to avoid passlib deprecation warnings.
